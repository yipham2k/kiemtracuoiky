                ...
                @foreach ($giohangs as $id => $giohang)
                    <tr>
                            ...
                            <form id="thanh-toan" action="/thanh-toan" method="POST">
                                ...
                                <div class="form-check info">
                                    <input class="form-check-input" type="checkbox" value="{{ $id }}"
                                        name="check-gio-hang[]" form="thanh-toan" checked />
                                </div>
                            </form>
                        </th>
                        <td scope="row">
                            <img src="../../../images2/{{ $giohang['hinh_anh_1'] ?? 'default.jpg' }}" alt="..."
                                class="img-fluid rounded-start" width="100px" />
                        </td>
                        <td>{{ $giohang['ten_giay'] ?? 'Sản phẩm' }}</td>
                        <td>{{ number_format($giohang['don_gia'] ?? 0) }} VNĐ</td>
                        ...
                        <form action="/gio-hang/cap-nhat" method="POST">
                                ...
                                <div class="d-flex">
                                    ...
                                    <div class="form-outline" style="width:80px">
                                        <input id="form1" min="1" name="so_luong"
                                            value="{{ $giohang['so_luong'] ?? 1 }}" type="number" autocomplete="off"
                                            class="form-control" />
                            ...
                            <td><b>{{ number_format(($giohang['so_luong'] ?? 1) * ($giohang['don_gia'] ?? 0)) }} VNĐ<b>
                            </td>
                            <td>
    ...
    <div class="card ">
        <form class="card-header">
            <div class="float-start">
                <h4 class="card-title text-success" style="margin-top: 20px">Tổng tiền: {{ number_format($tongtien) }}
                    VNĐ
                </h4>
            </div>
            <div class="float-end">
